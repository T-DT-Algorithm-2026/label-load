...
/// 检查和修复组件
///
/// 检查标签文件中的问题（如越界、重复）并自动修复。
class CheckAndFixWidget extends StatefulWidget {
  const CheckAndFixWidget({super.key});

  @override
  State<CheckAndFixWidget> createState() => _CheckAndFixWidgetState();
}

class _CheckAndFixWidgetState extends State<CheckAndFixWidget> {
  /// 选中的标签目录。
  String? _directory;
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        ElevatedButton.icon(
          onPressed: _isProcessing || _directory == null ? null : _startCheck,
          icon: const Icon(Icons.play_arrow),
          label: Text(_isProcessing ? l10n.processing : l10n.checkAndFix),
  ...
  Future<void> _startCheck() async {
    ...
    final l10n = AppLocalizations.of(context)!;
    try {
      final (success, failed) = await _gadgetService.checkAndFix(
        _directory!,
